...
async def node_similarity_search(
    ...
    records, _, _ = await driver.execute_query(
        ...
        + filter_query
        + f"""
            WITH n, vector.similarity.cosine(n.{key}, $search_vector) AS score
            WHERE score > $min_score"""
        + ENTITY_NODE_RETURN
        + """
        ORDER BY score DESC
...
async def get_embeddings_for_nodes(
    driver: AsyncDriver, nodes: list[EntityNode], key: Literal['name_embedding', 'summary_embedding'] = 'name_embedding'
) -> dict[str, list[float]]:
